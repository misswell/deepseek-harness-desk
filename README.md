...
当前版本：0.3.20。安装包请从 [GitHub Releases](https://github.com/misswell/deepseek-harness-desk/releases/latest) 下载。
...
为降低内存占用，应用提供多级内存释放策略（均在“设置 → 高级 → 内存与性能”中可开关）：关闭窗口到菜单栏/托盘时卸载 Harness 网页（释放 WebView 数百 MB 内存）；窗口失焦 60 秒后也自动卸载页面，回到窗口时重新加载。同时为 Harness 后端限制 V8 堆上限，避免长时间运行的会话把内存撑到数 GB。Harness 后台服务在这些情况下都保持运行。
