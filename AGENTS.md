...
- `v0.3.49`：补齐 Agent 提醒缺的两类语义事件，并修掉“一轮任务弹两条通知”。分类层新增 `api-session/error` 与 `api-session/removed`（已在 `~/.dsh/profiles/node_modules/@deepseek-ai/dsh-api-remotes` 的转发白名单里核实，签名分别是 `emit(sessionId, message)` 与 `emit(sessionId)`），`user-questions/request` 里 `questions[].intent.kind === 'plan-review'` 归为独立的 `NoticeKind::PlanReview`，不再和提问共用“需要你的输入”。完成通知不再在 `running → idle` 当帧直接投递：新增 `AttentionLedger` 把完成压进 `COMPLETION_DEBOUNCE`（300ms），并用 `attended_sessions` 记住“本轮已经提醒过”，只有会话重新 running 才解除，所以 `approval/request` 之后紧跟的 `status false` 只留“需要你的批准”，反过来窗口内才到达的交互也会顶掉完成，优先级 `error > interaction > completed`。两条读流（`remote.mux` 与 legacy `events.mux`/`events.host`）都改成 `tokio::select! { biased; }` 同时等 socket 帧与 deadline，`wait_until_deadline(None)` 用 `futures_util::future::pending` 挂起，状态机只在 `AttentionLedger::route` 一处。隐私：新增 `notify_detail`（默认关），问题原文与报错详情必须用户显式打开才进通知正文，`notify_error` 默认开；锁屏上只会看到“需要你的输入 / 计划等待你的确认 / 任务执行失败”。会话删除与后端退出分别按会话、按整体清理 ledger 状态，去重键仍跨重连保留（dsh 会重放仍待处理的请求）。测试：Rust 新增 `mux_error_*`/`mux_removed_*`/`plan_review_*`/`interaction_notices_report_the_session_they_came_from`/`first_idle_only_sets_a_baseline`/`one_completion_fires_*`/`an_interaction_covers_the_completion_of_the_same_turn`/`an_interaction_arriving_inside_the_window_replaces_the_completion`/`a_failure_covers_*`/`a_new_turn_clears_the_previous_one`/`a_removed_session_stops_producing_notices`/`pending_completions_are_tracked_per_session`/`a_replayed_interaction_banners_once`/`reset_forgets_the_previous_backend`；契约 `tests/notification-wiring.test.mjs` 钉住防抖常量、`attended_sessions`、baseline 判定、select 等待、两类新事件名与 error 开/detail 关的默认值，`tests/notification-prefs.test.mjs` 覆盖 error/detail 键。未做：通知内的“允许/拒绝/回答”按钮（waterfall 结算属于 Web 客户端，Native 只负责提醒）、“应用位于前台时也提醒”开关。另记：曾有一份针对旧 SwiftUI 工程的“WKWebView 注入 WebSocket observer”方案，要求 Desk 不建立第二条 `$events` 连接；该工程已不发布（`release.yml` 只构建 `tauri-app`），现行 Rust 独立连接已验证可用，不要按那份方案回退。
